...
int size[maxn], son[maxn], top[maxn], fa[maxn], dis[maxn], id[maxn], rid[maxn], End[maxn];
...
inline void DFS1(int u) {
    ...
    for (int i = Begin[u]; i; i = Next[i]) {
        ...
        dis[v] = dis[u] + 1; fa[v] = u; DFS1(v); size[u] += size[v];
...
inline void Build(int root, int l, int r) {
    if (l == r) { Tree[root] = g[rid[l]]; return; } int Mid = l + r >> 1;
...
inline void Modify(int root, int l, int r, int pos) {
    if (l == r) {Tree[root] = g[rid[l]]; return;}
    int Mid = l + r >> 1;
...
inline void Update(int x, int val) {
    ...
    while (x) {
        ...
        x = fa[top[x]];
        g[x].g[0][0] += max(now.g[0][0], now.g[1][0]);
        g[x].g[0][0] -= max(last.g[0][0], last.g[1][0]);
        g[x].g[0][1] = g[x].g[0][0];
...
int main() {
    ...
    REP(i, 1, n - 1) {
        int u, v; scanf("%d%d", &u, &v); (u, v); add(v, u);
    }
    dis[1] = 1; DFS1(1); DFS2(1, 1); Build(1, 1, n);
    REP(i, 1, m) {
        int x, val; scanf("%d%d", &x, &val); Update(x, val);
